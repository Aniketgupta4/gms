...
const MemberDetail = () => {
  ...
  const handleShare = async () => {
    ...
    const file = new File([blob], "member-card.png", {
      type: "image/png",
    });
  ...
  return (
    <div className='flex-1 min-h-screen bg-[#f8fafc] lg:pl-72 w-full'>
      ...
      <div className='p-6 lg:p-10 max-w-[1400px] mx-auto'>
        ...
        <div className="mb-10 flex items-center justify-between">
          ...
          <button 
            onClick={() => navigate(-1)} 
            className='flex items-center gap-2 px-6 py-3 bg-slate-900 text-white rounded-2xl text-xs'
          >
            <ArrowBackIcon sx={{ fontSize: 18 }} /> GO BACK
          </button>
        ...
        {/* 🔥 CARD WITH ID */}
        <div id="member-card" className='bg-white rounded-[3rem] border shadow-2xl'>

          <div className='flex flex-col lg:flex-row'>

            {/* LEFT */}
            <div className='lg:w-1/3 p-10 flex flex-col items-center'>
              <img 
                src={data?.profilePic || "https://i.pravatar.cc/300"} 
            ...
            <div className='lg:w-2/3 p-10'>
              ...
              <div className='grid grid-cols-2 gap-6 mt-6'>
                <InfoItem icon={<FiPhone />} label="Mobile" value={data?.mobileNo} />
                <InfoItem icon={<FiCalendar />} label="Joining" value={formatDate(data?.createdAt)} />
                <InfoItem icon={<FiMapPin />} label="Address" value={data?.address} />
                <InfoItem icon={<FiClock />} label="Next Bill" value={formatDate(data?.nextBillDate)} />
              </div>
            </div>
