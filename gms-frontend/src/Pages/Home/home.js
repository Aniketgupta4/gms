...
const Home = () => {
  ...
  const [isModalOpen, setIsModalOpen] = useState(false); // 🔷 Pop-up State
    ...
    <div className="min-h-screen bg-[#fafafa] text-slate-900 font-sans antialiased selection:bg-indigo-600 selection:text-white overflow-x-hidden">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 animate-in fade-in zoom-in duration-300">
          ...
          <div className="relative bg-white w-full max-w-lg rounded-[2.5rem] p-8 md:p-12 shadow-2xl text-center border border-white">
            ...
            <h2 className="text-3xl font-extrabold tracking-tight mb-2 text-slate-900">Partner With Us</h2>
            <p className="text-slate-500 font-medium mb-8">Want to list your gym on our platform? <br className="hidden md:block"/> Get in touch with our Admin to activate your premium membership.</p>

            <div className="bg-slate-50 border border-slate-100 p-6 rounded-3xl mb-8">
               <h4 className="text-xl font-black text-slate-800 italic uppercase tracking-tight">Aniket Gupta</h4>
               <p className="text-indigo-600 font-black text-2xl mt-1 tracking-tighter">+91 8770191425</p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
               <a href="tel:+918770191425" className="flex items-center justify-center gap-2 py-4 bg-slate-900 text-white rounded-2xl font-bold hover:bg-slate-800 transition-all active:scale-95 shadow-lg shadow-slate-900/20"><FiPhone /> Call Now</a>
               <a href="https://wa.me/918770191425" target="_blank" rel="noreferrer" className="flex items-center justify-center gap-2 py-4 bg-[#25D366] text-white rounded-2xl font-bold hover:opacity-90 transition-all active:scale-95 shadow-lg shadow-green-500/20"><FiMessageCircle /> WhatsApp</a>
            </div>
          </div>
        </div>
      )}
      ...
      <header className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled ? "bg-white/80 backdrop-blur-lg shadow-sm h-16 border-b border-slate-100" : "bg-transparent h-20 md:h-24"}`}>
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-full flex justify-between items-center">
          <div className="flex items-center gap-2.5 cursor-pointer group" onClick={() => scrollToSection("hero")}>
            ...
            <span className="text-xl font-extrabold tracking-tight text-slate-900">GymPro.</span>
          </div>
          <nav className="hidden md:flex items-center gap-8 font-semibold text-xs text-slate-500 uppercase tracking-widest">
            ...
            <button onClick={() => scrollToSection("auth")} className="px-5 py-2.5 bg-slate-900 text-white rounded-full hover:bg-slate-800 transition-all shadow-md font-bold tracking-tighter">Owner Login</button>
          </nav>
      ...
      <section id="hero" className="pt-32 pb-16 md:pt-48 md:pb-24 px-4 md:px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto grid lg:grid-cols-2 gap-12 items-center text-center lg:text-left">
          <div className="relative z-10">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-white border border-slate-200 text-slate-600 rounded-full text-[10px] font-bold tracking-widest uppercase mb-6 shadow-sm">
              <span className="w-2 h-2 bg-indigo-500 rounded-full animate-pulse" /> Enterprise Management OS
            </div>
            <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tighter leading-[1] mb-6 text-slate-900 uppercase">
              Run gyms <br /><span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 to-blue-500 italic">Digitally.</span>
            </h1>
            <p className="text-base md:text-lg text-slate-500 max-w-lg mx-auto lg:mx-0 leading-relaxed font-medium mb-10">
            ...
            <div className="flex flex-wrap justify-center lg:justify-start gap-4">
                <button onClick={() => scrollToSection("auth")} className="group px-7 py-4 bg-slate-900 text-white font-black rounded-full flex items-center gap-2 hover:bg-indigo-600 transition-all shadow-xl active:scale-95 uppercase text-xs tracking-widest">
          ...
          <div className="relative hidden lg:block transform lg:rotate-2">
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full bg-indigo-100 rounded-full blur-[80px] opacity-70" />
      ...
      <section id="features" className="py-20 md:py-24 bg-white px-4 md:px-6 border-y border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="max-w-2xl mb-16 text-center lg:text-left">
            <h2 className="text-3xl md:text-4xl font-black tracking-tighter mb-4 text-slate-900 uppercase italic">The Business Engine.</h2>
            <p className="text-slate-500 text-base md:text-lg font-medium">Everything you need to scale your fitness business, built into one seamless platform.</p>
          </div>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              ...
              <div key={i} className="p-8 bg-[#fafafa] rounded-[2rem] border border-slate-100 hover:shadow-xl transition-all duration-300 group">
      ...
      {/* 🔷 MEMBERSHIP ARCHITECT SECTION (RESTORED & PRO) */}
      <section id="plans" className="py-20 md:py-24 px-4 md:px-6 bg-[#fafafa]">
        <div className="max-w-7xl mx-auto">
          <div className="text-center mb-16 space-y-4">
             <h2 className="text-4xl lg:text-6xl font-black tracking-tighter italic uppercase text-slate-900">Membership Architect.</h2>
             <p className="text-slate-400 font-bold uppercase tracking-[0.2em] text-xs">Create flexible revenue streams</p>
          </div>
      ...
      <section id="metrics" className="py-20 md:py-24 px-4 md:px-6 bg-slate-900 text-white relative overflow-hidden text-center">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          <div className="mb-16 max-w-2xl mx-auto">
            <h2 className="text-3xl lg:text-5xl font-black tracking-tighter mb-4 text-white uppercase italic">Built for Scale.</h2>
            <p className="text-slate-400 font-medium text-lg px-4">Trusted by elite fitness centers to handle operations securely and fast.</p>
          </div>
      ...
      {/* 🔷 AUTH SECTION */}
      <section id="auth" className="py-20 md:py-24 px-4 md:px-6 bg-[#fafafa]">
        <div className="max-w-6xl mx-auto bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-slate-100 flex flex-col lg:flex-row">
          <div className="lg:w-[45%] bg-slate-900 p-12 text-white flex flex-col justify-between text-left relative">
            ...
            <div className="relative z-10">
              ...
              <h3 className="text-4xl font-black italic leading-none mb-6 uppercase">Command <br /><span className="text-indigo-500 text-6xl">Center.</span></h3>
              <p className="text-slate-400 text-base font-medium leading-relaxed italic">Login to oversee admissions, track expiry, and scale your gym business.</p>
            </div>
              ...
              {authMode === "protect" && (
                <div className="space-y-6 text-left animate-in fade-in duration-300">
                   <div className="p-4 bg-indigo-50 rounded-2xl border border-indigo-100 flex items-center gap-3">
                      ...
                      <p className="text-[10px] font-black text-indigo-800 uppercase tracking-tight italic">Identity Verification Required</p>
                   </div>
                   <div className="space-y-2">
                     ...
                     <input 
                        ...
                        className="w-full px-4 py-4 bg-slate-50 border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-100 transition-all font-bold"
                     />
                   </div>
      ...
      <footer className="bg-white pt-24 pb-12 px-6 border-t border-slate-100">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-20 text-center md:text-left">
            <div className="md:col-span-2">
              <div className="flex items-center justify-center md:justify-start gap-2 mb-6">
                ...
                <span className="text-2xl font-black tracking-tight text-slate-900 italic uppercase">GymPro.</span>
              </div>
              <p className="text-slate-500 text-sm font-medium leading-relaxed max-w-sm mx-auto md:mx-0">
            ...
            <div>
              ...
              <ul className="space-y-4 text-xs font-black text-slate-400 uppercase">
                ...
                <li className="hover:text-indigo-600 cursor-pointer" onClick={() => scrollToSection("auth")}>Admin Portal</li>
              </ul>
            </div>
            <div>
          ...
          <div className="pt-10 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 text-center">
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em]">© 2026 GymPro Technologies. All rights reserved.</p>
            <p className="text-slate-400 text-[10px] font-black uppercase tracking-[0.2em] italic">Built with Elite Status by Aniket Gupta</p>
          </div>
        </div>
      </footer>
    </div>
