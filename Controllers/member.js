...
exports.getAllMember = async(req,res) => {
    try{
        ...
        res.status(200).json({
            message:members.length?"Fetched Members Successfully":"No any Member Regustered yet",
...
function addMonthsToDate(months, joiningDate) {
  ...
  // Adjust day if current day exceeds days in future month
...
exports.registerMember = async(req,res) => {
    try{
        ...
        if(member){
            return res.status(400).json({error:"Member with this mobile number already exists"})
        }
        ...
        if(memberShip){
            ...
            res.status(200).json({message:"Member registered successfully", member:newmember})
        }else{
            return res.status(400).json({error:"No such Membership Plan are there"})
        }
...
exports.searchMember = async(req,res) => {
  try{
    ...
    res.status(200).json({
        message:member.length?"Fetched Members Successfully":"No Such Member Found Yet",
        members:member,
  ...
  try {
    ...
    res.status(200).json({
      message: member.length
        ? "Members expiring within 3 days fetched successfully"
